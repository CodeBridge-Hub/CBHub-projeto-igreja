...
const InputField = ({ label, id, type = "text", placeholder, value, onChange, inputMode, pattern, required = false }) => (
    <div className="flex flex-col w-full">
        <label htmlFor={id} className={`text-sm font-medium mb-1 ${TEXT_DARK_BLUE}`}>{label}</label>
        <input
            type={type}
            id={id}
            value={value}
            ...
            {options.map((option) => (
                <div key={option.id} className="flex items-center">
                    <input
                        ...
                        className="h-4 w-4 rounded border-gray-300 accent-[#253965] focus:ring-[#253965] focus:border-[#253965]"
                    />
...
const CadastroVoluntario = () => {
    ...
    const validateForm = () => {
        const requiredFields = ['nome', 'email', 'data_nascimento', 'cpf', 'telefone', 'habilidades'];

        for (const field of requiredFields) {
            if (!formData[field]) {
                setValidationError(`O campo "${field.toUpperCase().replace('_', ' ')}" é obrigatório.`);
                return false;
            }
        }

        if (formData.areas.length === 0) {
            setValidationError("Selecione pelo menos uma 'Área de Atuação'.");
            return false;
        }

        if (formData.dias_disponiveis.length === 0) {
            setValidationError("Selecione pelo menos um 'Dia de Disponibilidade'.");
            return false;
        }

        if (!formData.turno) {
            setValidationError("Selecione um 'Turno' de disponibilidade.");
            return false;
        }

        if (formData.areas.includes('outro_area') && !formData.outro_campo_desc) {
            setValidationError("Por favor, descreva sua 'Outra Área de Atuação'.");
            return false;
        }
    ...
    const handleSubmit = (e) => {
        ...
        if (validateForm()) {
            console.log("Dados do Voluntário Enviados:", formData);
            ...
            <main className="flex-grow flex justify-center pb-12 px-4">
                <div className="w-full max-w-4xl bg-white shadow-xl rounded-lg p-6 md:p-10 border border-gray-200">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                        ...
                        <div className="border-b pb-6">
                            <CheckboxGroup
                                title="Área de Atuação (Selecione uma ou mais)"
                                name="areas"
                            ...
                            {formData.areas.includes('outro_area') && (
                                <div className="mt-3">
                                    <label htmlFor="outro_campo_desc" className={`text-xs ${TEXT_DARK_BLUE}`}>Defina sua área de atuação (ex: Serviços gerais)</label>
                                    <input
                                        type="text"
                                        id="outro_campo_desc"
                                        placeholder="Por exemplo: Serviços gerais."
                                        value={formData.outro_campo_desc}
                                        onChange={handleInputChange}
                                        className={`w-full p-2 border border-gray-300 rounded-md text-sm mt-1 focus:ring-[#253965] focus:border-[#253965]`}
                                    />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-b pb-6">
                            <CheckboxGroup
                                title="Disponibilidade (Selecione um ou mais)"
                                name="dias_disponiveis"
                                options={disponibilidadeOptions}
                                onCheckChange={handleCheckboxChange}
                                values={formData.dias_disponiveis}
                            />
                            <RadioGroup
                                title="Turno:"
                                name="turno"
                        ...
                        <div>
                            <label htmlFor="observacoes" className={`text-sm font-medium mb-1 ${TEXT_DARK_BLUE}`}>OBSERVAÇÕES ADICIONAIS <span className="text-gray-500 font-normal">(Opcional)</span></label>
                            <textarea
                                id="observacoes"
                                rows="3"
